...
class UIUtils {
    ...
    companion object{
        fun getScreenWidthDp(context: Context): Float {
            ...
            return width / (if (scale <= 0) 1.0F else scale) + 0.5f
        }
        ...
        fun getStatusBarHeight(context: Context): Float {
            var height = 0f
            val resourceId: Int = context.applicationContext.resources.getIdentifier("status_bar_height", "dimen", "android")
            if (resourceId > 0) {
        ...
        fun px2dip(context: Context, pxValue: Float): Float {
            val scale: Float = context.resources.displayMetrics.density
            return (pxValue / (if (scale <= 0) 1.0F else scale) + 0.5f)
        }
        ...
        fun isAndroidPHasNotch(activity: Activity?): Boolean {
            ...
            if (Build.VERSION.SDK_INT >= 28) {
                try {
                    ...
                    val method: Method = windowInsets.getMethod("getDisplayCutout")
                    val displayCutout: Any = method.invoke(windowInsets)
        ...
        fun getInt(key: String?, activity: Activity): Int {
            ...
            if (isMiui()) {
                try {
                    ...
                    val getInt: Method = SystemProperties.getMethod("getInt", *paramTypes)
                    //参数
